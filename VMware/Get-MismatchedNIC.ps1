...
<#
    ...
    clusters, or all VMs in the vCenter. For each VM, every network interface
	is checked to determine if it is currently connected and if it is set to
	connect when the VM starts. This is important as a NIC that is currently
	connected could unexpectidly become disconnected after a reboot of the VM.

    true  = a mismatch has been detected
    false = the configuration of both connection states is identical

    If there is a mismatch detected but the VM is currently powered off (and
	the NIC is set to connect at power on), this is counted as a flase positive
	...
	.PARAMETER cluster
    The cluster to gather VMs from. You can specify one, or an array of
	clusters. If no parameters are set it is assumed you are supplying the
	name, or names, of a cluster.
	...
	.PARAMETER all
    This switch will gather all VMs registered to the currently connected
	vCenter Server, regardless of name or the cluster it resides within.
